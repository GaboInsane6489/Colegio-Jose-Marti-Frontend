import { FaUserGraduate, FaChartBar, FaClipboardList, FaUserCircle } from 'react-icons/fa';

/**
 * 🎓 Componente institucional para mostrar resumen de rendimiento de un estudiante
 */
const ResumenEstudiante = ({ estudiante, entregas = [] }) => {
  if (!estudiante || !(estudiante._id || estudiante.id)) return null;

  const entregasEstudiante = entregas.filter(
    (e) => e.estudianteId?._id === estudiante._id || e.estudianteId === estudiante.id
  );
    ...
    <div className='bg-white/90 text-gray-900 rounded-xl shadow-md p-6 space-y-6'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm'>
        ...
        <div className='flex items-center gap-2'>
          ...
          <span>
            Promedio: <strong>{isNaN(promedio) ? '—' : `${promedio.toFixed(2)}/20`}</strong>
          </span>
        </div>
      </div>
    </div>
