...
const NotaCard = ({ entrega, onEdit }) => {
  ...
  const { estudianteId, calificacion, comentarioDocente, estado, fechaEntrega } = entrega;
    ...
    <div className='bg-white/90 text-gray-900 rounded-xl shadow-lg p-5 space-y-4'>
      ...
      <div className='flex justify-between items-center'>
        ...
        <span
          className={`px-3 py-1 text-xs rounded-full font-semibold ${
            estado === 'revisado' ? 'bg-lime-500 text-black' : 'bg-yellow-400 text-black'
          }`}
        >
      ...
      <div className='space-y-2 text-sm'>
        <div className='flex items-center gap-2'>
          ...
          <span>
            Calificación:{' '}
            <strong>{typeof calificacion === 'number' ? `${calificacion}/20` : 'Sin nota'}</strong>
          </span>
        </div>
      ...
      {modoEdicion && (
        <div className='mt-4'>
          <NotaForm entrega={entrega} onClose={() => setModoEdicion(false)} onEdit={onEdit} />
        </div>
      )}
    </div>
  );
