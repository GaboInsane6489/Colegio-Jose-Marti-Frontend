...
const EntregasList = ({ actividadId }) => {
  ...
  useEffect(() => {
    ...
    const fetchEntregas = async () => {
      setLoading(true);
      try {
        const { data } = await axiosInstancia.get(`/api/docente/entregas/${actividadId}`); // ✅ ruta institucional

        if (Array.isArray(data.entregas)) {
          // Normalizamos IDs: siempre usar "id"
          const limpias = data.entregas.map((e) => ({
            ...e,
            id: e.id || e._id, // preferimos id, fallback a _id
          }));

          console.log('📥 Entregas recibidas:', limpias);
          setEntregas(limpias);
          setError(null);
        } else {
        ...
        setError(err.response?.data?.msg || 'No se pudieron cargar las entregas');
      } finally {
        setLoading(false);
      }
    };
        ...
        return (
          ...
          >
            ...
            <p className='text-sm text-white/80 mb-1'>
              📅 Fecha de entrega: <strong className='text-white'>{fechaFormateada}</strong>
            </p>

            {entrega.calificacion !== undefined && (
              <p className='text-sm text-white/80 mb-1'>
                🧮 Calificación: <strong className='text-white'>{entrega.calificacion}/20</strong>
              </p>
            )}
            ...
            {entrega.comentarioDocente && (
              <p className='text-sm text-white/80 mb-1'>
                💬 Comentario:{' '}
                <span className='italic text-white'>{entrega.comentarioDocente}</span>
              </p>
            )}
