    ...
    <div className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
      {loadingEntregas ? (
        ...
        <div className='bg-yellow-100 text-yellow-800 p-4 rounded-lg col-span-full text-center border border-yellow-300'>
          <p className='font-semibold'>Sin entregas que coincidan con los filtros seleccionados.</p>
          <p className='text-sm mt-1'>
            Prueba cambiar la materia o el lapso para ver otras entregas.
          </p>
        </div>
      ) : (
        entregasFiltradas.map((e) => <ActividadDetalleEstudiante key={e._id} entrega={e} />)
      )}
    </div>

    {/* Mensaje adicional si no hay entregas en absoluto */}
    {!loadingEntregas && entregasFiltradas.length === 0 && (
