...
const ValoresSection = () => {
  ...
  return (
    <section
      ref={ref}
      className="pt-10 pb-20 px-4 sm:px-6 bg-[#fdfdfd] rounded-xl"
    >
      <motion.h2
        initial={{ opacity: 0, y: 30 }}
        animate={inView ? { opacity: 1, y: 0 } : {}}
        transition={{ type: "spring", bounce: 0.4, duration: 0.8 }}
        className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-10 sm:mb-12 text-[#1a1a1a] drop-shadow-[0_0_6px_#ccc]"
      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 sm:gap-10 max-w-6xl mx-auto">
        {valores.map((valor, index) => (
          <motion.div
            ...
            className="bg-[#1a1a1a] rounded-xl shadow-lg p-6 sm:p-8 text-center border border-[#333] hover:shadow-[0_0_16px_#ffffff] transition-shadow duration-300"
          >
            <div className="text-[#ffffff] text-3xl sm:text-4xl mb-3 sm:mb-4 flex justify-center drop-shadow-[0_0_6px_#ffffff]">
              {valor.icon}
            </div>
            <h3 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4 text-white drop-shadow-[0_0_4px_#ffffff]">
              {valor.title}
            </h3>
