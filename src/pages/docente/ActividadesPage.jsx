...
const ActividadesPage = () => {
  ...
  const handleNuevaActividad = (nuevaActividad) => {
    ...
    if (actividadEditando) {
      setActividades((prev) =>
        prev.map((act) =>
          act._id === actividadFinal._id ? actividadFinal : act
        )
      );
  ...
  const totalPaginas = Math.ceil(
    actividadesFiltradas.length / actividadesPorPagina
  );
      ...
      <main className="relative z-20 px-6 py-24 max-w-6xl mx-auto space-y-10">
        <div className="text-center">
          ...
          <p className="text-white/70 text-lg">
            Crea, organiza y publica actividades para tus cursos. Puedes incluir
            fechas, recursos, materia y criterios de evaluación.
          </p>
        </div>
